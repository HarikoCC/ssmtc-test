...
def get_cache_directories():
    ...
    for entry in os.listdir("."):
        # 保留原修复：os.isdir → os.path.isdir（解决报错崩溃）
...
def main():
    parser = argparse.ArgumentParser(description="全Cache兼容测试指令生成工具-SMT3专用版")
    parser.add_argument("-s", "--scenario", type=int, required=True, help="1-3工况，1:1+1+1 2:1+2 3:3线程")
    ...
    # 完全保留原中文工况映射风格
    scenario_map = {1:"1+1+1", 2:"1+2", 3:"3线程"}

    print("="*60)
    print(" 测试指令生成工具 - 全目录通用单份清单 | SMT3专用版")
