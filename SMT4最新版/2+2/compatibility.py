...
# ================= 配置区域 =================
TEST_TIMEOUT = 180
PROBLEMATIC_FILES = {"smalltrans.arg2"}
...
def generate_all_combinations(file_dict, scenario_type):
    ...
    if scenario_type == 1:
        if len(file_dict[1]) < 4:
            ...
            sys.exit(1)
        for comb in itertools.combinations(file_dict[1], 4):
        ...
        if len(file_dict[2]) < 2:
            ...
            sys.exit(1)
        # 🔥 修复2：修正变量名错误 c → comb（核心报错点）
        for comb in itertools.combinations(file_dict[2], 2):
            if is_combination_valid(comb):
                combinations.append(comb)
