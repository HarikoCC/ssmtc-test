...
def validate_and_generate_combinations(file_dict, scenario_type):
    ...
    if scenario_type == 1:
        ...
        if len(file_dict[1]) < 3:
            print(f"错误：工况1（1+1+1）需要至少3个.arg文件，当前仅找到{len(file_dict[1])}个")
            sys.exit(1)
        for candidate_comb in itertools.combinations(file_dict[1], 3):
            if is_combination_valid(candidate_comb):
                combinations.append(candidate_comb)
        
    elif scenario_type == 2:
        # 工况2：1+2
        # 顺序调整为：[.arg2, .arg] (多线程优先)
        if len(file_dict[1]) < 1:
            print(f"错误：工况2（1+2）需要至少1个.arg文件，当前仅找到{len(file_dict[1])}个")
            sys.exit(1)
        if len(file_dict[2]) < 1:
            print(f"错误：工况2（1+2）需要至少1个.arg2文件，当前仅找到{len(file_dict[2])}个")
            sys.exit(1)
        
        for f_arg, f_arg2 in itertools.product(file_dict[1], file_dict[2]):
            # 重新排序：把 .arg2 放在前面
            candidate_comb = (f_arg2, f_arg)
            if is_combination_valid(candidate_comb):
                combinations.append(candidate_comb)
        ...
        if len(file_dict[3]) < 1:
            print(f"错误：工况3（3）需要至少1个.arg3文件，当前仅找到{len(file_dict[3])}个")
            sys.exit(1)
        combinations = [(f,) for f in file_dict[3]]
        
    else:
        print("错误：无效的工况类型！请选择1-3之间的数字")
        sys.exit(1)
...
def run_single_test(combination, combination_id):
    ...
    scenario_tag = f"smt3混合工况_{len(combination)}进程"
...
def main():
    parser = argparse.ArgumentParser(description="SMT-3混合负载自动化测试工具 (多线程优先启动版)")
    parser.add_argument(
        "-s", "--scenario", 
        type=int, 
        required=True, 
        help="测试工况编号（1-3）：1=1+1+1（3个单线程）、2=1+2（1单1双）、3=3（1个三线程）"
    )
    parser.add_argument(
        "-j", "--jobs", 
        type=int, 
