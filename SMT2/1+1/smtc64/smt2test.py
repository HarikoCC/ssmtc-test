...
def get_file_base_name(filename):
    """
    提取文件的基础程序名，去除.arg/.arg2后缀
    """
    if filename.endswith(".arg2"):
        return filename[:-5]
...
def validate_and_generate_combinations(file_dict, scenario_type):
    ...
    if scenario_type == 1:
        ...
        if len(file_dict[1]) < 2:
            print(f"错误：工况1（1+1）需要至少2个.arg文件，当前仅找到{len(file_dict[1])}个")
            sys.exit(1)
        for candidate_comb in itertools.combinations(file_dict[1], 2):
        ...
        if len(file_dict[2]) < 1:
            print(f"错误：工况2（2）需要至少1个.arg2文件，当前仅找到{len(file_dict[2])}个")
            sys.exit(1)
        combinations = [(f,) for f in file_dict[2]]
        
    else:
        print("错误：无效的工况类型！请选择1-2之间的数字")
        sys.exit(1)
...
def run_single_test(combination, combination_id):
    ...
    scenario_tag = f"smt2混合工况_{len(combination)}进程"
...
def main():
    parser = argparse.ArgumentParser(description="SMT-2混合负载自动化测试工具")
    parser.add_argument(
        "-s", "--scenario", 
        type=int, 
        required=True, 
        help="测试工况编号（1-2）：1=1+1（2个单线程）、2=2（1个双线程）"
    )
    parser.add_argument(
        "-j", "--jobs", 
        type=int, 
