...
def validate_and_generate_combinations(file_dict, scenario_type):
    ...
    # Also sort this mixed pool for consistency
    pool_1thread_plus_argn = sorted(list(set(pool_1thread_plus_argn)))

    if scenario_type == 1:
        # Scenario 1: 1+1+1+1 (4x from mixed pool [.arg, .argn])
        ...
        # Scenario 2: 1+1+2
        # Logic: 2 from mixed pool (.arg/.argn), 1 from .arg2
        ...
        # Scenario 3: 1+3
        # Logic: 1 from mixed pool (.arg/.argn), 1 from .arg3
    ...
    # Filter: Keep only combinations that contain at least one .argn file
    if not file_dict['n']:
    ...
    for comb in combinations:
        has_argn = any(f.endswith(".argn") for f in comb)
        if has_argn:
    ...
    if not combinations:
        print("Error: No valid test combinations containing .argn could be generated!")
        print("       (Check if .argn files share base names with other required files)")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="SMT-4 Mixed Load Automation Tool (Argn-Only Mode)")
    parser.add_argument(
        "-s", "--scenario", 
        type=int, 
        required=True, 
    ...
    print("  [MODE: Testing only combinations containing .argn]")
    ...
    print(f"Total Valid Combinations (w/ .argn): {len(combinations)}")
