...
def main():
    ...
    tee_logger = None
    try:
        parser = argparse.ArgumentParser(description="SMT-4混合负载自动化测试工具 (含总结日志版)")
        parser.add_argument(
            "-s", "--scenario", 
            type=int, 
            required=True, 
        ...
        summary_log_path = os.path.join("./logs", f"test_summary_{start_timestamp}.log")
        tee_logger = TeeLogger(summary_log_path)
        ...
        print(f"日志保存目录：./logs")
        print(f"本次测试总结文件：{summary_log_path}")
        ...
        if failed_test_list:
            print(f"  失败的测试组合（已尝试所有排列）：")
            for failed_comb in failed_test_list:
                print(f"    - {failed_comb}")
        print("="*50)
        print(f"测试总结已保存至：{summary_log_path}")

    finally:
        # 【新增】确保无论脚本如何结束，都恢复标准输出并关闭日志文件
        if tee_logger:
