...
def parse_smt_result(file_path):
    ...
    # 【修改】改用字典独立记录每个 MASTER 的 hits 和 accesses
    master_stats = {}
    ...
    for line in lines:
        ...
        if current_section == "GLOBAL_PERFORMANCE":
            ...
            if current_section not in master_stats:
                master_stats[current_section] = {"hits": 0, "accesses": 0}
            if key == "hits":
                master_stats[current_section]["hits"] += val
            if key == "accesses":
                master_stats[current_section]["accesses"] += val
    ...
    # 【修改】计算每个 MASTER 的独立命中率，然后取算术平均
    master_hit_ratios = []
    for stats in master_stats.values():
        if stats["accesses"] > 0:
            master_hit_ratios.append(stats["hits"] / stats["accesses"] * 100)
        else:
            # 无访存请求的 MASTER 不计入平均，避免稀释有效命中率
            # 若需严格包含所有 MASTER（计为0%），可改为 master_hit_ratios.append(0.0)
            pass

    data["hit_ratio"] = sum(master_hit_ratios) / len(master_hit_ratios) if master_hit_ratios else 0.0
    return data
...
def main():
    ...
    # 导出CSV
    csv_file = "avg_multi_cache_smt_compare.csv"
